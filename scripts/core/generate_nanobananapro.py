#!/usr/bin/env python3
"""
Google Gemini 3 Pro Image parallel generator with adaptive rate limiting.
Generates panels for D&D Graphic Novel.
"""
...
def build_character_prompt_section(char_name, characters_db):
    ...
    if desc_components and len(desc_components) > 1:
        ...
        if desc_components.get('physical'):
            parts.append(f"  {desc_components['physical']}")
        if desc_components.get('visual'):
...
async def generate_panel_async(panel, page_num, client, characters_db, locations_db, style_db):
    """Generate a single panel with Gemini (with retry logic)."""
...
async def main():
    ...
    logger.info("=" * 70)
    logger.info("GEMINI COMIC GENERATION")
    logger.info("=" * 70)
